...
/**
 * COMPROBANTE DE INGRESO (Individual)
 */
export const generateEntryReceipt = async (order, clientSignatureBase64, appLogo) => {
  ...
  doc.setFontSize(10);
  doc.setFont("helvetica", "bold");
  doc.text("DETALLES DEL INGRESO", 15, 50);
  doc.line(15, 52, 195, 52);

  const tableData = [
    ["Orden ID:", order.id, "Fecha Ingreso:", order.entryDate],
    ["Cliente:", order.clientName, "Teléfono:", order.clientPhone || "N/D"],
    ["Equipo:", order.deviceType || order.equipmentName, "Marca/Mod:", order.brandModel || order.brand],
  ...
  doc.setFont("helvetica", "bold");
  doc.text("FALLA REPORTADA / OBSERVACIONES:", 15, nextY);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  const description = order.issueDescription || order.observations || "Sin descripción detallada.";
  const splitDesc = doc.splitTextToSize(description, 180);
  doc.text(splitDesc, 15, nextY + 6);

  const accY = nextY + 10 + (splitDesc.length * 5);
  doc.setFont("helvetica", "bold");
  doc.text("ACCESORIOS RECIBIDOS:", 15, accY);
  doc.setFont("helvetica", "normal");
  doc.text(order.accessories?.join(", ") || "Ninguno", 15, accY + 6);

  // Terms
  renderTermsAndConditions(doc, accY + 15);
...
export const generateBudgetPDF = async (order, appLogo) => {
  ...
  if (appLogo) {
    try { doc.addImage(appLogo, 'JPEG', 170, 5, 25, 25); } catch (e) {}
  }
...
export const exportWorkOrdersToPDF = (orders) => {
  ...
  doc.text(settings.companyName.toUpperCase(), 15, 18);
  doc.setFontSize(10);
  doc.text("REPORTE GENERAL DE ÓRDENES DE TRABAJO", 15, 27);
  ...
  doc.text(settings.pdfFooter, 15, 285);
...
export const exportInventoryToPDF = (inventory) => {
  ...
  const columns = ["ID", "Insumo", "Categoría", "Stock", "Precio"];
  const rows = inventory.map(item => [item.id, item.name, item.category || "General", item.stock, `$${item.price.toLocaleString()}`]);
...
export const generateQCCertificate = (order) => {
  ...
  doc.text("Firma y Sello Lab", 135, sigY + 5);
